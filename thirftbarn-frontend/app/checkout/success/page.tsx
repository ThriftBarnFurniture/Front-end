...
export default function CheckoutSuccessPage() {
        ...
        const baseQuery = supabase
          ...
          .select(
            [
              ...
              "shipping_address",
              "stripe_email",
              "stripe_session_id", // (optional but handy)
            ].join(",")
          );
  ...
  // Finalize checkout (your existing call)
  useEffect(() => {
    if (!sessionId) return;
  ...
  const items: any[] = useMemo(() => {
    ...
    // items can be array of objects OR array of JSON strings
    return arr
      .map((x) => {
        if (!x) return null;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ignore: page still works without images
