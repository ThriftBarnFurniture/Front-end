...
export async function POST(req: Request) {
  try {
    ...
    // Admin check (adjust if your admin field/table differs)
    ...
    if ((percent_off === null && amount_off === null) || (percent_off !== null && amount_off !== null)) {
      return NextResponse.json(
        { error: "Set exactly one: percent_off OR amount_off" },
        { status: 400 }
      );
    }
    ...
    if (starts_at === "INVALID") {
      return NextResponse.json({ error: "starts_at is not a valid date" }, { status: 400 });
    }
    if (ends_at === "INVALID") {
      return NextResponse.json({ error: "ends_at is not a valid date" }, { status: 400 });
    }
    ...
    if (error) {
      // If you have a unique constraint on code, this will catch duplicates
